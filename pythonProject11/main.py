def accept_set(A, Str):
    n = int(input("\nEnter the total no. of student who play %s : " % Str))
    for i in range(n):
        x = input("Enter the name of student %d who plays %s : " % ((i + 1), Str))
        A.append(x)
...
def Main():
    ...
    while True:
        ...
        if (ch == 6):
            ...
            print("Number of students who play neither cricket nor badminton = %s\n" % len(Group_R))
        elif (ch == 5):
            display_set(Group_A, "Cricket")
            display_set(Group_C, "Football")
            display_set(Group_B, "Badminton")
